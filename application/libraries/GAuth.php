...
class GAuth
{
    ...
    {
        if (!is_numeric($range)) {
            log_message('error', $this->lang->line('gauth_invalid_range'));
    ...
    public function setRefresh($seconds)
    {
        if (!is_numeric($seconds)) {
            log_message('error', $this->lang->line('gauth_invalid_refresh'));
    ...
    public function validateCode($code, $initKey = null, $timestamp = null, $range = null)
    {
        if (strlen($code) !== $this->getCodeLength()) {
            log_message('error', $this->lang->line('gauth_invalid_code_lenght'));
            show_error($this->lang->line('gauth_invalid_code_lenght'));
        }
    ...
    public function base32_decode($hash)
    {
        ...
        if (preg_match('/^['.implode('', array_keys($lookup)).']+$/', $hash) == false) {
            log_message('error', $this->lang->line('gauth_invalid_base32'));
            show_error($this->lang->line('gauth_invalid_base32'));
        }
    ...
    public function _assign_libraries()
    {
        if($CI =& get_instance())
        {
            ...
            $this->lang->load('gauth');

            return;
        }

        log_message('error', $this->lang->line('gauth_instance_na'));
        show_error($this->lang->line('gauth_instance_na'));
    }
}
